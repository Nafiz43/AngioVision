...
UPDATED:
- On-the-fly frame subsampling added.
- By default, keeps 20% of available frames per sequence using uniform skipping.
- Does NOT delete, move, or modify any files on disk.
- Sampling happens only during reading/loading for training.
"""
...
# ------------------------------------------------------------
def uniformly_sample_percent_frames(
    frame_files: List[Path],
    keep_percent: float = 0.20,
    min_keep: int = 1,
) -> List[Path]:
    """
    Keep a uniform subset of frames using skipping / evenly spaced selection.

    Example:
      - 100 frames, keep_percent=0.20 -> keep ~20 frames
      - 10 frames, keep_percent=0.20 -> keep 2 frames
      - 3 frames, keep_percent=0.20 -> keep at least min_keep frames

    This does NOT modify files on disk. It only returns a sampled list of paths.
    """
    n = len(frame_files)
    if n == 0:
        return []
    ...
    if keep_percent <= 0:
        return frame_files[:min_keep]

    keep_n = max(min_keep, int(math.ceil(n * keep_percent)))
    keep_n = min(keep_n, n)

    if keep_n >= n:
        return frame_files[:]

    idxs = torch.linspace(0, n - 1, steps=keep_n).long().tolist()
    # Deduplicate just in case torch.linspace rounds repeated values on very small n
    idxs = sorted(set(idxs))

    # If dedup reduced count below keep_n, fill from remaining indices
    if len(idxs) < keep_n:
        used = set(idxs)
        for i in range(n):
            if i not in used:
    ...
    return [frame_files[i] for i in idxs]


def uniformly_cap_frames(frame_files: List[Path], max_frames: Optional[int]) -> List[Path]:
    """
    Uniformly cap frame count if max_frames_per_sequence is set.
    """
    if max_frames is None or len(frame_files) <= max_frames:
        return frame_files

    idxs = torch.linspace(0, len(frame_files) - 1, steps=max_frames).long().tolist()
    return [frame_files[i] for i in idxs]
...
class StudyDataset(Dataset):
    def __init__(
        ...
        frame_keep_percent: float = 0.20,
        min_sampled_frames_per_sequence: int = 1,
        ...
        self.frame_keep_percent = frame_keep_percent
        self.min_sampled_frames_per_sequence = min_sampled_frames_per_sequence
    ...
    def __getitem__(self, idx: int):
        ...
        for sop in sop_uids:
            ...
            # ----------------------------------------------------
            # NEW: On-the-fly keep only 20% of available frames
            # ----------------------------------------------------
            frame_files = uniformly_sample_percent_frames(
                frame_files=frame_files,
                keep_percent=self.frame_keep_percent,
                min_keep=self.min_sampled_frames_per_sequence,
            )

            # Optional additional hard cap after the 20% sampling
            frame_files = uniformly_cap_frames(
                frame_files=frame_files,
                max_frames=self.max_frames_per_sequence,
            )
...
def train(args):
    ...
    print(f"[INFO] On-the-fly frame keep percent: {args.frame_keep_percent:.4f}")
    print(f"[INFO] Minimum sampled frames per sequence: {args.min_sampled_frames_per_sequence}")
    ...
    dataset = StudyDataset(
        ...
        frame_keep_percent=args.frame_keep_percent,
        min_sampled_frames_per_sequence=args.min_sampled_frames_per_sequence,
...
def build_argparser():
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="If set, uniformly sample at most this many frames per SOP/sequence after percent-based sampling.",
    )

    # NEW: on-the-fly frame keep percentage
    ap.add_argument(
        "--frame_keep_percent",
        type=float,
        default=0.20,
        help="Fraction of frames to keep per sequence on the fly (default: 0.20 = 20%%). No files are modified.",
    )
    ap.add_argument(
        "--min_sampled_frames_per_sequence",
        type=int,
        default=1,
        help="Minimum number of sampled frames to keep per sequence after percent-based sampling.",
    )
