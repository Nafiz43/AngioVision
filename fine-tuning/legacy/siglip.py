#!/usr/bin/env python3
"""
custom_framework_train.py  (flat-batch + SigLIP edition)

AngioVision fine-tuning: (Study) frames -> ViT -> temporal-aware pooling -> visual embedding
Align visual embedding with report text embedding using SigLIP sigmoid loss.

Text encoder: UCSD-VA-health/RadBERT-RoBERTa-4m  (default, overridable via CLI)

OPTIMISATIONS applied
──────────────────────────────────────────────────────────────────────────────
1.  Frame-path cache             – find_frame_files_for_sop results cached in
                                   StudyDataset.__init__; __getitem__ never
                                   touches the filesystem.

2.  Vectorised report_map build  – groupby instead of row-by-row loop.

3.  PIL image loading in threads – ThreadPoolExecutor opens images concurrently.

4.  In-place running pool        – torch scatter-reduce helpers; chunked only
                                   where needed for grad-memory safety.

5.  Buffered loss CSV writer     – deque buffer, flushes every flush_every steps.

6.  Cosine-with-warmup scheduler – linear warmup + cosine decay.

7.  AMP scaler guarded           – GradScaler only when use_amp=True.

8.  Deterministic worker seeding – worker_init_fn seeds numpy/random/torch.

9.  Gradient accumulation fix    – correct flush on final batch.

10. torch.cuda.empty_cache throttled – only every N steps.

11. Non-blocking pin_memory transfer.

12. Auto-detect ViT native image size – resolve_vit_image_size().

13. Train command persistence    – train_cmd.sh + train_config.txt in run_dir.

14. Flat-batch frame encoding (NEW)
    forward() collects ALL frame paths from ALL studies and ALL sequences in
    the mini-batch into one flat list, performs a single parallel I/O burst
    (ThreadPoolExecutor), then runs frames through ViT in large chunks
    (e.g. frame_chunk_size=128) instead of 144 tiny calls of 12 frames each.
    Sequence boundaries are preserved via a (study_idx, seq_idx, local_frame_idx)
    registry; scatter-back after ViT assigns each embedding to its originating
    sequence before pooling.  encode_framepaths_pooled() is kept for external
    use by the validation script.
    Typical speedup: 3-5x per training step.

15. SigLIP sigmoid loss (NEW)
    Replaces the softmax-based symmetric CLIP loss with an independent binary-
    classification loss over all B^2 (image, text) pairs
    (Zhai et al. 2023, "Sigmoid Loss for Language-Image Pre-Training").
    A learnable siglip_bias parameter (init=-10.0) is added alongside the
    existing logit_scale.  Works well at small batch sizes; no requirement for
...
but state dicts will now include the siglip_bias parameter.
"""
...
def _list_images_in_dir(d: Path) -> List[Path]:
    """Return sorted image paths in *d*."""
    ...
    def _build_frame_cache(self) -> None:
        for _, row in self.meta.iterrows():
            ...
            for sop in sop_uids:
                ...
                frame_files = list(self._frame_cache[key])
...
def resolve_vit_image_size(vit_name: str, override: Optional[int] = None) -> int:
    ...
    Resolution priority:
      ...
      4. Hard-coded per-model fallback table.
...
class PooledCLIP(nn.Module):
    """
    ViT visual encoder + text encoder aligned via SigLIP sigmoid loss.

    Architecture
    ────────────
    • Vision:  ViT (RAD-DINO by default) -> CLS token -> vision_proj -> L2-norm
    • Text:    AutoModel (RadBERT by default) -> CLS token -> text_proj -> L2-norm
    • Loss:    SigLIP (see siglip_loss); logit_scale + siglip_bias are learnable.

    Forward (flat-batch)
    ─────────────────────
    All frames from all studies and sequences in the mini-batch are collected
    into one flat list, loaded in a single I/O burst, then run through ViT in
    large chunks.  A (study_idx, seq_idx, local_frame_idx) registry preserves
    sequence boundaries so pooling happens correctly per-sequence.
    """
    ...
    def __init__(
        ...
        # SigLIP learnable parameters
        # logit_scale: temperature (same init as CLIP, fine for SigLIP)
        # siglip_bias: per-pair offset, init=-10.0 as in the original paper
    ...
    # ── running pool helpers (used by encode_framepaths_pooled) ───────────
    ...
    # ── single-sequence encoder (kept for validation script compatibility) ─
    ...
    def encode_framepaths_pooled(
        ...
        """Encode all frames for one SOP/sequence, return a pooled (D,) vector.

        Kept for external use by the validation script; training uses the
        flat-batch forward() instead.
        """
    ...
    def forward(
        ...
        """
        Flat-batch forward pass.

        All frames from all studies and all sequences in the mini-batch are
        collected into a single flat list.  One ThreadPoolExecutor I/O burst
        loads all images.  ViT runs on large chunks (frame_chunk_size) that
        span the entire batch rather than one tiny sequence at a time.

        Sequence boundaries are preserved via a (study_idx, seq_idx,
        local_frame_idx) registry.  The scatter-back step groups embeddings by
        (study_idx, seq_idx) before pooling, so no frames from different
        sequences are ever pooled together.

        Returns
        ───────
        image_embeds : (B, E)  – L2-normalised vision projection
        text_embeds  : (B, E)  – L2-normalised text projection
        logit_scale  : scalar  – exp-clamped temperature
        siglip_bias  : scalar  – learnable per-pair bias (SigLIP)
        """
        B = len(batch_sequences)
        assert B == len(texts), "Batch size mismatch between images and texts"

        # ── Step 1: build flat registry ───────────────────────────────────
        # registry[i] = (study_idx, seq_idx, local_frame_idx_within_seq, path)
        # This is the source-of-truth for scatter-back after ViT.
        registry: List[Tuple[int, int, int, Path]] = []
        for si, seqs in enumerate(batch_sequences):
            for qi, frame_paths in enumerate(seqs):
                for fi, p in enumerate(frame_paths):
                    registry.append((si, qi, fi, p))

        # ── Step 2: one parallel I/O burst for the entire mini-batch ─────
        # Images for all 12×12×12 = 1728 frames loaded at once instead of
        # 144 separate per-sequence loads.
        loaded: List[Optional[Image.Image]] = [None] * len(registry)
        with ThreadPoolExecutor(max_workers=io_threads) as ex:
            fut_map = {ex.submit(self._load_image, r[3]): i for i, r in enumerate(registry)}
            for fut in as_completed(fut_map):
                img = fut.result()
                if img is not None:
                    loaded[fut_map[fut]] = img

        valid_entries: List[Tuple[int, Image.Image]] = [
            (i, img) for i, img in enumerate(loaded) if img is not None
        ]

        # ── Step 3: ViT in large chunks across the whole batch ───────────
        # e.g. frame_chunk_size=128 → ~14 ViT calls instead of 144.
        # Gradient flows through embs[local_pos] views back to the ViT
        # parameters because we never detach.
        all_embs: List[Optional[torch.Tensor]] = [None] * len(registry)
        vit_ctx = torch.no_grad() if self._vit_is_frozen() else torch.enable_grad()

        for c_start in range(0, len(valid_entries), frame_chunk_size):
            chunk        = valid_entries[c_start: c_start + frame_chunk_size]
            chunk_idxs   = [t[0] for t in chunk]
            chunk_imgs   = [t[1] for t in chunk]
            ...
            pixel_values = processor(**proc_kwargs)["pixel_values"].to(device, non_blocking=True)
            with vit_ctx:
                embs = self.vit(pixel_values=pixel_values).last_hidden_state[:, 0, :]  # (C, D)

            # Store per-frame embedding; embs rows are views into the same
            # underlying storage → autograd graph is preserved after del embs.
            for local_pos, g_idx in enumerate(chunk_idxs):
                all_embs[g_idx] = embs[local_pos]

            del pixel_values, embs, chunk_imgs

        # ── Step 4: scatter back → pool frames → pool sequences ───────────
        # study_seq_embs[si][qi]   : list of (D,) tensors for that sequence
        # study_seq_fidxs[si][qi]  : list of local frame indices (for temporal)
        #
        # Sequence boundaries are explicit: qi is the sequence index within
        # study si.  Frames from seq 0 and seq 1 are never mixed.
        study_seq_embs:  Dict[int, Dict[int, List[torch.Tensor]]] = \
            defaultdict(lambda: defaultdict(list))
        study_seq_fidxs: Dict[int, Dict[int, List[int]]] = \
            defaultdict(lambda: defaultdict(list))

        for reg_idx, (si, qi, fi, _) in enumerate(registry):
            emb = all_embs[reg_idx]
            if emb is not None:
                study_seq_embs[si][qi].append(emb)
                study_seq_fidxs[si][qi].append(fi)

        study_visuals: List[torch.Tensor] = []
        for si in range(B):
            seq_feats: List[torch.Tensor] = []
            # sorted() preserves original sequence order (qi=0,1,2,…)
            for qi in sorted(study_seq_embs[si].keys()):
                emb_list = study_seq_embs[si][qi]
                fidxs    = study_seq_fidxs[si][qi]
                if not emb_list:
                    continue

                seq_stack = torch.stack(emb_list, dim=0)  # (n_frames, D)

                if self.temporal_on_frames and self.temporal_mode != "none":
                    pos_t = torch.tensor(fidxs, device=device, dtype=torch.long)
                    seq_stack = self._add_temporal_encoding(
                        seq_stack, pos_t, self.frame_temporal_scale
                    )

                seq_feats.append(pool_stack(seq_stack, self.frame_pooling))  # (D,)

            if not seq_feats:
                study_visuals.append(torch.zeros(self.vit_hidden, device=device))
            else:
                seq_stack = torch.stack(seq_feats, dim=0)  # (n_seqs, D)
                ...
                study_visuals.append(pool_stack(seq_stack, self.sequence_pooling))  # (D,)

        # ── Step 5: project + normalise ────────────────────────────────────
        study_visuals_t = torch.stack(study_visuals, dim=0)                      # (B, D_vit)
        image_embeds    = F.normalize(self.vision_proj(study_visuals_t), dim=-1)  # (B, E)

        # ── Step 6: text encoding ──────────────────────────────────────────
        tok = tokenizer(
            texts, padding=True, truncation=True, max_length=512, return_tensors="pt"
        ).to(device)
        text_ctx = torch.no_grad() if self._text_is_frozen() else torch.enable_grad()
        with text_ctx:
            tcls = self.text_model(**tok).last_hidden_state[:, 0, :]              # (B, D_txt)

        text_embeds = F.normalize(self.text_proj(tcls), dim=-1)                   # (B, E)
        logit_scale = self.logit_scale.exp().clamp(1e-3, 100.0)

        return image_embeds, text_embeds, logit_scale, self.siglip_bias


# ─────────────────────────────────────────────────────────────────────────────
# SigLIP loss  (replaces CLIP softmax loss)
# ─────────────────────────────────────────────────────────────────────────────
...
def siglip_loss(
    ...
    Each of the B² (image_i, text_j) pairs is an independent binary
    classification:
      y_ij = +1  if i == j  (positive pair)
      y_ij = -1  if i != j  (negative pair)

    Loss = -mean_{i,j}  log σ( y_ij · (t · <img_i, txt_j> + b) )

    where t = logit_scale (learnable temperature),
          b = siglip_bias  (learnable per-pair offset, init=-10.0).

    Compared with CLIP softmax loss:
      • Each pair gradient is independent → more stable at small B.
      • No requirement for large in-batch negatives.
      • The bias absorbs the natural positive/negative imbalance in each row.
      • Scales gracefully: loss magnitude is O(1) regardless of B.
    """
    B      = image_embeds.size(0)
    device = image_embeds.device
    dtype  = image_embeds.dtype

    # Build B×B cosine similarity matrix (chunked to bound peak memory)
    ...
    # Mean sigmoid cross-entropy over all B² pairs
...
def print_trainable_summary(model: PooledCLIP) -> None:
    ...
    print(f"[INFO] logit_scale trainable:         {model.logit_scale.requires_grad}  (value={model.logit_scale.item():.4f})")
...
def build_optimizer(model: PooledCLIP, args) -> torch.optim.Optimizer:
    ...
    head_params = (
        ...
        + ([model.siglip_bias] if model.siglip_bias.requires_grad else [])  # SigLIP bias
    )
...
def train(args) -> None:
    ...
    loss_csv_path = run_dir / _loss_csv_name(
    ...
    print(
        f"[INFO] Effective vit_image_size = {effective_vit_image_size}  "
    ...
    # Optional: torch.compile for ~15-25% additional throughput (PyTorch >= 2.0)
    if args.torch_compile and not args.cpu and hasattr(torch, "compile"):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with amp_ctx:
                image_embeds, text_embeds, logit_scale, s_bias = model(
            ...
            pbar.set_postfix(
                ...
                ema=f"{ema:.4f}",
                bs=len(batch["text"]),
                bias=f"{s_bias.item():.3f}",
            )

            del loss, loss_to_bp, image_embeds, text_embeds, logit_scale, s_bias
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--torch_compile",      action="store_true",
                    help="Enable torch.compile(mode='reduce-overhead'). Requires PyTorch >= 2.0.")

    # ── data loading ──────────────────────────────────────────────────────
    ap.add_argument("--num_workers",    type=int, default=4)
    ap.add_argument("--prefetch_factor",type=int, default=2)
    ap.add_argument("--io_threads",     type=int, default=4,
                    help="ThreadPoolExecutor workers for parallel image loading per batch.")
    ap.add_argument("--cpu",            action="store_true")

    # ── sequence / frame limits ───────────────────────────────────────────
    ap.add_argument("--frame_chunk_size",        type=int,  default=128,
                    help="ViT batch size within the flat-batch forward. "
                         "With flat-batching, this spans ALL sequences in the mini-batch "
                         "(e.g. 128 gives ~14 ViT calls for 1728 total frames vs 144 previously).")
    ...
    ap.add_argument("--logits_chunk",     type=int, default=4,
                    help="Row chunk size when building the B×B similarity matrix in siglip_loss.")
