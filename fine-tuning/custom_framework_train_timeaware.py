...
NEW (Option B: temporal awareness without real timestamps):
- Adds a learnable frame position (time) embedding to each frame CLS embedding
  BEFORE pooling across frames. This lets the model know frames are sequential.
- Controlled by:
    --max_frames_pos (default: 1024)
  Frames beyond this length are clamped to the last embedding index.
"""
...
class PooledCLIP(nn.Module):
    def __init__(
        ...
        max_frames_pos: int = 1024,  # NEW: temporal position embedding capacity
    ):
        super().__init__()
        ...
        if max_frames_pos <= 0:
            raise ValueError(f"max_frames_pos must be > 0, got {max_frames_pos}")
        ...
        # NEW: learnable frame "time"/position embedding
        # Frames with index >= max_frames_pos are clamped to max_frames_pos-1
        self.max_frames_pos = int(max_frames_pos)
        self.frame_pos_emb = nn.Embedding(self.max_frames_pos, self.vit_hidden)
    ...
    def encode_frames_pooled(
        ...
        """
        NEW: adds learnable position embedding based on each frame's index (0..T-1)
        before pooling across frames.
        """
        running, aux = self._init_running(device, self.vit_hidden, pooling)

        # Process in chunks to reduce VRAM
        for i in range(0, len(frame_images), chunk_size):
            ...
            frame_emb = out.last_hidden_state[:, 0, :]  # (chunk_len, vit_hidden) CLS

            # --- NEW: add time/position embedding for each frame index ---
            # Global frame indices for this chunk: i..i+chunk_len-1
            chunk_len = frame_emb.size(0)
            pos_ids = torch.arange(i, i + chunk_len, device=device, dtype=torch.long)
            pos_ids = pos_ids.clamp_(0, self.max_frames_pos - 1)
            frame_emb = frame_emb + self.frame_pos_emb(pos_ids)
            # ------------------------------------------------------------
...
def train(args):
    ...
    print(f"[INFO] max_frames_pos (time embedding size) = {args.max_frames_pos}")
    ...
    model = PooledCLIP(
        ...
        max_frames_pos=args.max_frames_pos,  # NEW
    ).to(device)
...
def build_argparser():
    ...
    # NEW: time embedding capacity
    ap.add_argument(
        "--max_frames_pos",
        type=int,
        default=1024,
        help="Size of learnable frame position/time embedding table. Frame indices >= this value are clamped.",
    )
