...
Updated to use BioBERT by default via Hugging Face AutoModel/AutoTokenizer.
Default text encoder:
    dmis-lab/biobert-base-cased-v1.1

Small temporal workaround added:
- Preserves frame order inside each sequence by injecting deterministic sinusoidal
  positional encodings into frame CLS embeddings before frame pooling.
- Optionally also injects sequence-order encodings before study-level pooling.
...
NEW UPDATE:
- reports_csv may contain MULTIPLE reports per accession (e.g., Original + Augmented 1..4)
- For each accession, all available reports are collected
- At __getitem__ time, one report is sampled UNIFORMLY at random from the available reports
- If only one report exists for an accession, it still works normally
"""
...
class StudyDataset(Dataset):
    def __init__(
        ...
        # accession -> list of report variants
        ...
        # Optional filtering: keep only metadata rows whose accession has >=1 usable report
        ...
        # Precompute counts for visibility/debugging
        ...
        if n_accessions_with_reports > 0:
            ...
            preview_items = sorted(self.report_count_map.items(), key=lambda x: x[0])[:10]
            print("[INFO] First few accession -> #reports examples:")
            for acc, cnt in preview_items:
                print(f"       {acc} -> {cnt}")
        else:
    ...
    def _sample_report_uniformly(self, reports_for_acc: List[Dict[str, str]], idx: int) -> Dict[str, str]:
        """
        Uniformly sample one report variant for this accession.

        If only one exists, return it directly.
        If multiple exist, sample with equal probability.

        Uses torch.randint for worker-safe sampling behavior.
        """
        ...
        # Uniform sampling across available reports
    ...
    def __getitem__(self, idx: int):
        ...
        for sop in sop_uids:
            ...
            # Uniformly subsample while preserving order
...
def build_sinusoidal_position_encoding(
    ...
    """
    positions: (N,) integer or float tensor
    returns:   (N, dim)
    """
    if positions.ndim != 1:
...
def build_argparser():
    ...
    ap.add_argument(
        ...
        type=str,
        default="Type",
        help="Column in reports_csv indicating whether the report is Original / Augmented 1 / ...",
    )
    ap.add_argument(
        "--report_sampling",
        type=str,
        default="uniform",
        choices=["uniform"],
        help="How to sample among multiple report variants for the same accession.",
    )
    ap.add_argument(
        "--report_sampling_seed",
        type=int,
        default=42,
        help="Reserved seed parameter for report sampling behavior.",
    )
    ...
    ap.add_argument(
        ...
        help="Text encoder model name. Default is BioBERT. You can still override this from CLI.",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="If set, uniformly sample at most this many frames per SOP/sequence to reduce memory/time while preserving order.",
    )
    ...
    ap.add_argument(
        ...
        help="How to inject order information. 'sinusoidal' is deterministic and lightweight.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Also add positional encoding across sequence order within a study.",
    )
    ap.add_argument(
        "--frame_temporal_scale",
        type=float,
        default=0.25,
        help="Scale of frame-order positional encoding added before frame pooling.",
    )
    ap.add_argument(
        "--sequence_temporal_scale",
        type=float,
        default=0.25,
        help="Scale of sequence-order positional encoding added before study pooling.",
    )
...
#     # Partial ViT fine-tuning controls
#     ap.add_argument(
#         "--vit_trainable_blocks",
#         type=int,
#         default=3,
