...
────────
  ...
  3. Embed ALL TRAIN frames with microsoft/rad-dino (model loaded ONCE).
     Ingest into a single in-memory ChromaDB EphemeralClient — no disk I/O.

  4. For each TEST sequence (K=1 retrieval):
       • Extract + embed up to --max-frames frames (uniform sampling)
       • Batch-query ChromaDB K=1 per frame → (retrieved_sequence_id, label)
       • Majority-vote across all frame results → predicted sequence
       • Hit ↔ predicted sequence's angio_run == true angio_run
...
─────
  ...
  python eval_k1_retrieval.py --scale-down 0.8 --max-frames 10 --seed 42
...
─────────
  ...
  --max-frames   INT    Max frames sampled per sequence for both
                        ingestion and querying        [default: 10]
...
def extract_frames(path_str: str, max_frames: int = 0) -> list[np.ndarray]:
    """
    Open a DICOM file and return up to max_frames uniformly-sampled
    uint8 RGB frames.  Returns [] on any error.
    """
    try:
    ...
    # Normalise shape → (T, H, W) or (T, H, W, C)
    if pixels.ndim == 2:
...
def ingest_train_sequences(
    ...
    """
    Embed and ingest ALL categories' TRAIN sequences into the collection.
    Metadata per frame: sequence_id, angio_run, frame_idx.
    Returns total frames ingested.
    """
    train_seqs = [seq for _, (train, _) in splits.items() for seq in train]
    log.info(f"Ingesting {len(train_seqs):,} TRAIN sequences …")
    ...
    with tqdm(total=len(train_seqs), unit="seq", desc="  Ingesting TRAIN") as pbar:
        for seq in train_seqs:
            frames = extract_frames(seq["dicom_path"], max_frames=max_frames)
            if not frames:
                skipped_seqs += 1
                pbar.update(1)
                continue
            ...
            collection.add(
                embeddings=embs,
                ids=[f"{stem}_f{i:05d}" for i in range(len(embs))],
                metadatas=[
                    {
                        "sequence_id": stem,
    ...
    log.info(
        f"Ingestion done — {total_frames:,} frames stored "
        f"({skipped_seqs} sequences had no extractable frames)"
    )
    return total_frames


# ═══════════════════════════════════════════════════════════════════════════════
# 7 ── K=1 sequence-level evaluation
# ═══════════════════════════════════════════════════════════════════════════════
...
def evaluate_one_sequence(
    ...
    max_frames: int,
    embed_batch: int,
) -> Optional[tuple[bool, str, str, Counter]]:
    """
    Evaluate a single test sequence.

    Cross-sequence frame matching strategy
    ───────────────────────────────────────
    Each test frame is embedded and used to query ChromaDB K=1.
    The retrieved frame carries its parent sequence_id and angio_run label.
    We accumulate a vote for that sequence_id across all query frames, then
    select the sequence_id with the highest vote count as the predicted
    sequence.  The predicted label is that sequence's angio_run.

    Returns (hit, true_label, predicted_label, vote_counter) or None if
    ...
    # Batch query: one ChromaDB round-trip for all frame embeddings
    ...
    seq_votes:  Counter           = Counter()   # sequence_id → vote count
    seq_labels: dict[str, str]    = {}          # sequence_id → angio_run
    ...
    return hit, true_label, predicted_label, seq_votes
...
def run_evaluation(
    ...
    all_test = [
        seq
        for _, (_, test) in splits.items()
        for seq in test
    ]
    log.info(f"Evaluating {len(all_test):,} TEST sequences …")

    with tqdm(total=len(all_test), unit="seq", desc="  Evaluating TEST") as pbar:
        for seq in all_test:
            label   = seq["angio_run"]
            outcome = evaluate_one_sequence(
                seq, collection, model, processor, device,
                max_frames, embed_batch,
            )
            ...
            if outcome is None:
                ...
                hit, true_lbl, pred_lbl, _ = outcome
                r["total_evaled"] += 1
                r["correct"]      += int(hit)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "Max frames uniformly sampled per sequence (both ingestion "
            "and querying). Higher = more accurate but slower.  [default: 10]"
        ),
    )
    parser.add_argument(
    ...
    log.info("═" * 60)
    log.info("  AngioVision K@1 Retrieval Evaluation")
    log.info("═" * 60)
    log.info(f"  scale-down    : {args.scale_down:.0%} train / "
             f"{1 - args.scale_down:.0%} test")
    log.info(f"  max-frames    : {args.max_frames} per sequence")
    ...
    n_frames_stored = ingest_train_sequences(
        splits, collection, model, processor, device,
        args.max_frames, args.embed_batch,
    )
