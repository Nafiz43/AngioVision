...
────────
  1. Load labeled_DSA_2023_10_24.csv; skip rows where angio_run contains
     ...
     The First_Diag_Image and Last_Diag_Image columns (1-indexed frame numbers)
     define the diagnostic window parsed for each row.
  ...
  3. For each TRAIN sequence, extract only the diagnostically-relevant frames
     (First_Diag_Image … Last_Diag_Image, inclusive) and embed each with
     microsoft/rad-dino (model loaded ONCE).
     Ingest into a single in-memory ChromaDB EphemeralClient — no disk I/O.

  4. For each TEST sequence (K=1 retrieval):
       • Extract its diagnostic-window frames and embed each (1 vec/frame)
       • Batch-query ChromaDB K=1 per frame → (retrieved_sequence_id, label)
       • Majority-vote across frame results → predicted sequence
       • Hit ↔ predicted sequence's angio_run == query sequence's angio_run
...
Note: each sequence contributes only its First_Diag_Image→Last_Diag_Image
      frames to both ingestion and querying (fallback: all frames when the
...
def load_labeled_csv_grouped(csv_path: Path) -> dict[str, list[dict]]:
    ...
    Each seq_dict: accession, series_uid, run_type, angio_run, file_path,
                   first_diag_idx, last_diag_idx  (0-indexed; -1 = missing)

    First_Diag_Image / Last_Diag_Image are 1-indexed floats (e.g. 8.0, 11.0).
    We convert to 0-indexed by subtracting 1.
    When both are missing/invalid the full frame range is used as a fallback.
    """
    ...
    with open(csv_path, newline="", encoding="utf-8-sig") as fh:
        ...
        first_key = col("first_diag_image")   # ← NEW
        last_key  = col("last_diag_image")    # ← NEW

        if not path_key:
            raise ValueError("'file_path' column not found in labeled CSV")

        def _parse_1idx(raw: str) -> int:
            """Parse a 1-indexed float string to a 0-indexed int; -1 on failure."""
            try:
                val = float((raw or "").strip())
                return int(val) - 1 if val >= 1 else -1
        ...
        for row in reader:
            ...
            # ── Parse diagnostic window → 0-indexed ───────────────────────
            first_diag_idx = _parse_1idx(row.get(first_key, "") if first_key else "")
            last_diag_idx  = _parse_1idx(row.get(last_key,  "") if last_key  else "")

            # Both must be valid and ordered; otherwise mark as missing
            if first_diag_idx < 0 or last_diag_idx < 0 or last_diag_idx < first_diag_idx:
            ...
            groups[angio].append({
                ...
                "angio_run":     angio,
                "file_path":     fp,
                "first_diag_idx": first_diag_idx,   # ← NEW
                "last_diag_idx":  last_diag_idx,    # ← NEW
            })
    ...
    if n_missing_diag:
        log.warning(
            f"  {n_missing_diag:,} sequences have missing/invalid "
            f"First_Diag_Image / Last_Diag_Image — will use all frames as fallback"
        )
    return dict(groups)
...
# ═══════════════════════════════════════════════════════════════════════════════
# 5 ── DICOM frame extraction  (diagnostic window only)
# ═══════════════════════════════════════════════════════════════════════════════
...
def extract_diag_frames(
    path_str: str,
    first_idx: int,
    last_idx: int,
) -> list[np.ndarray]:
    """
    Extract the diagnostic window of frames [first_idx, last_idx] inclusive
    from a DICOM file and return them as uint8 RGB arrays.

    first_idx / last_idx are 0-indexed (already converted from the CSV's
    1-indexed First_Diag_Image / Last_Diag_Image values).

    Fallback rules:
      • Either index == -1 (missing/invalid) → extract ALL frames
      • Index out of range for this file     → clamp to valid bounds
    ...
    # Resolve the window — fall back to the full range when missing
    if first_idx < 0 or last_idx < 0:
        lo_idx, hi_idx = 0, total - 1
    else:
        lo_idx = max(0, min(first_idx, total - 1))
        hi_idx = max(0, min(last_idx,  total - 1))
        if hi_idx < lo_idx:
            hi_idx = lo_idx      # degenerate window → single frame
    ...
    for idx in range(lo_idx, hi_idx + 1):
        ...
        if raw.ndim == 3:
            ...
            frames.append(_to_uint8_rgb(raw, photometric))
    return frames
...
def ingest_train_sequences(
    ...
    """
    For each TRAIN sequence, extract its First_Diag_Image→Last_Diag_Image
    frames, embed each with RAD-DINO, and add to ChromaDB.
    Frame IDs are '{stem}_f{dicom_frame_idx:05d}' — stable across re-runs.
    Returns total frames ingested.
    """
    train_seqs = [seq for _, (train, _) in splits.items() for seq in train]
    log.info(
        f"Ingesting {len(train_seqs):,} TRAIN sequences "
        f"(diagnostic window frames each) …"
    )
    ...
    with tqdm(total=len(train_seqs), unit="seq", desc="  Ingesting TRAIN") as pbar:
        for seq in train_seqs:
            first_idx = seq["first_diag_idx"]
            last_idx  = seq["last_diag_idx"]
            if first_idx < 0 or last_idx < 0:
                fallback_count += 1

            frames = extract_diag_frames(seq["dicom_path"], first_idx, last_idx)
            if not frames:
                skipped_seqs += 1
                pbar.update(1)
                continue

            embs = embed_frames(frames, model, processor, device, embed_batch)
            stem = seq["stem"]

            # Frame index in ChromaDB matches the actual DICOM position
            dicom_start = max(0, first_idx) if first_idx >= 0 else 0
                ...
                metadatas=[
                    {
                        ...
                        "frame_idx":      dicom_start + i,
                        "first_diag_idx": first_idx,
                        "last_diag_idx":  last_idx,
                    }
                    for i in range(len(embs))
                ],
            )
            total_frames += len(embs)
            pbar.update(1)
            pbar.set_postfix(frames=total_frames, skip=skipped_seqs)

    log.info(
        f"Ingestion done — {total_frames:,} frames stored across "
        f"{len(train_seqs) - skipped_seqs:,} sequences | "
        f"{skipped_seqs} unreadable skipped | "
        f"{fallback_count} used full-range fallback"
    )
    return total_frames


# ═══════════════════════════════════════════════════════════════════════════════
# 7 ── K=1 sequence-level evaluation  (diagnostic window frames)
# ═══════════════════════════════════════════════════════════════════════════════
...
def evaluate_one_sequence(
    ...
    """
    Evaluate a single test sequence using its diagnostic-window frames.

    Cross-sequence frame matching strategy
    ───────────────────────────────────────
    Each test frame (First_Diag_Image … Last_Diag_Image) is embedded and
    used to query ChromaDB K=1.  The retrieved frame carries its parent
    sequence_id and angio_run label.  We accumulate a vote for that
    sequence_id across all frame queries, then select the sequence_id with
    the most votes as the predicted sequence.

    Hit ↔ predicted sequence's angio_run == query sequence's angio_run.

    Returns (hit, true_label, predicted_label) or None if no frames
    could be extracted.
    """
    first_idx = seq["first_diag_idx"]
    last_idx  = seq["last_diag_idx"]
    ...
    # Single ChromaDB round-trip for all diagnostic frame embeddings
    ...
    # Accumulate per-sequence votes
    seq_votes:  Counter        = Counter()
    seq_labels: dict[str, str] = {}
    ...
    predicted_label = seq_labels[best_sid]
...
def run_evaluation(
    ...
    log.info(
        f"Evaluating {len(all_test):,} TEST sequences "
        f"(diagnostic window frames, majority vote) …"
    )
...
def main() -> None:
    ...
    log.info("═" * 60)
    log.info("  AngioVision K@1 Retrieval Evaluation  [Diag-Window mode]")
    log.info("═" * 60)
    log.info(f"  scale-down    : {args.scale_down:.0%} train / "
             f"{1 - args.scale_down:.0%} test")
    log.info(f"  frame mode    : First_Diag_Image → Last_Diag_Image (majority vote)")
