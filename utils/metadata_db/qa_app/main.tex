...
over the relational metadata and report store, with results streamed via
server-sent events.\\
...
default) served through Ollama; \textbf{(b)}~image-similarity retrieval, in which
...
Both pathways stream progress to the browser via server-sent events (SSE).

\paragraph{Agentic query orchestration.}
Initial versions employed a single-shot pipeline: one LLM call generated SQL,
execution proceeded with up to two automatic repairs, and a second call
synthesized an answer. The current implementation replaces this with an agentic
orchestrator built on smolagents~\cite{huggingface2025smolagents}. The model
...
agent is equipped with a single read-only SQL tool that spans both the
structured metadata and the free-text reports---the latter searched with
\texttt{LIKE} predicates on the report column---and it issues this tool through
structured function calls rather than free-form code. With it the agent explores
the schema, samples distinct column values, recovers from SQL errors, and chains
multiple queries: for example, searching reports for a finding, extracting the
matching accession numbers, then aggregating the corresponding metadata. This
enables multi-hop queries that the single-shot pipeline could not resolve.
Content-based image retrieval is exposed as a separate, deterministic endpoint
rather than as an agent tool. All reasoning executes locally on the selected
Qwen3 model.
...
Two representative scenarios illustrate the system's workflow; both are
...
applications~\cite{thirunavukarasu2023llmmed,weinert2025rag}. The transition
...
\begin{figure}[ht]
  ...
  iterative reason--act--observe loops over a single read-only SQL tool. The
...
Multi-step reason--act--observe loop over a single read-only SQL tool spanning
metadata and free-text reports; SSE streaming &
