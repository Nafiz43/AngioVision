...
1) Sequence extraction (uses Ground Truth):
   ...
   - Reads GT CSV with columns (names can vary; see candidates below):
       ...
       * "SeriesInstanceUIDs" (comma-separated list)
   - For each report:
       * Looks up GT-ordered SeriesInstanceUIDs and N
...
2) Per-sequence QA (second LLM call per sequence):
   ...
   QA prompt includes BOTH:
     - verbatim_text (the ONLY source for clinical findings + evidence)
     - rationale (for boundary/context only, NOT evidence)

NEW TWEAK (FILTERING):
- Only processes reports whose GT sequence_instance_uids contain ANY UID found in:
  /data/Deep_Angiography/AngioVision/utils/report_extracted_exact_labels_y_n.csv
- The script reads that CSV, collects UIDs, and filters the reports list BEFORE calling the LLM.
- Reports missing from GT cannot be reliably filtered by UID; by default, they are SKIPPED.
...
python extract_sequences_and_sequence_QA_using_GT.py \
  ...
  --filter_csv /data/Deep_Angiography/AngioVision/utils/report_extracted_exact_labels_y_n.csv \
...
# NEW: filter CSV with sequence_instance_uid column
DEFAULT_FILTER_CSV_PATH = Path("/data/Deep_Angiography/AngioVision/utils/report_extracted_exact_labels_y_n.csv")
...
# NEW: filter CSV UID column candidates
FILTER_UID_COL_CANDIDATES = [
...
# -----------------------------
def load_allowed_uids(filter_csv: Path) -> Tuple[set, str]:
    """
    Reads filter_csv and returns:
      allowed_uids: set[str]
      uid_col_used: str
    """
    if not filter_csv.exists():
        raise FileNotFoundError(filter_csv)

    df = pd.read_csv(filter_csv)

    # try exact matches first
    uid_col = find_first_existing_column(df, FILTER_UID_COL_CANDIDATES)
    if uid_col is None:
        # try case-insensitive match
        lower_map = {c.lower(): c for c in df.columns}
        for cand in FILTER_UID_COL_CANDIDATES:
            if cand.lower() in lower_map:
                uid_col = lower_map[cand.lower()]
                break

    if uid_col is None:
        raise ValueError(
            f"Filter CSV missing a recognizable UID column. "
            f"Tried candidates: {FILTER_UID_COL_CANDIDATES}. "
            f"Columns present: {list(df.columns)}"
        )

    allowed = set()
    for v in df[uid_col].tolist():
        if v is None:
            continue
        ...
        s = str(v).strip()
        if s:
            # if any cell has comma-separated values, split them too
            parts = [p.strip() for p in s.split(",")]
            for p in parts:
                if p:
                    allowed.add(p)
    ...
    if not allowed:
        raise ValueError(f"No UIDs found in filter CSV column '{uid_col}'.")

    return allowed, uid_col


def report_has_any_allowed_uid(gt_entry: Optional[Dict[str, Any]], allowed_uids: set) -> bool:
    if not gt_entry or not gt_entry.get("uids"):
        return False
    uids = gt_entry.get("uids", [])
    return any((u in allowed_uids) for u in uids)
...
def main():
    ...
    parser.add_argument(
        "--filter_csv",
        type=Path,
        default=DEFAULT_FILTER_CSV_PATH,
        help="CSV containing sequence_instance_uid values; only reports whose GT uids intersect this set are processed.",
    )
    parser.add_argument(
        "--include_gt_missing",
        action="store_true",
        help="If set, also process reports whose Anon Acc # is missing from GT (cannot UID-filter those). Default: skip them.",
    )
    ...
    if not args.filter_csv.exists():
        raise FileNotFoundError(args.filter_csv)
    ...
    # NEW: load allowed UIDs from filter CSV
    allowed_uids, filter_uid_col_used = load_allowed_uids(args.filter_csv)

    # NEW: filter reports BEFORE processing
    def should_keep_row(r: pd.Series) -> bool:
        anon_acc = normalize_scalar(r.get(ANON_ACC_COL))
        gt_entry = gt_lookup.get(anon_acc) if anon_acc else None

        if gt_entry and gt_entry.get("uids"):
            return report_has_any_allowed_uid(gt_entry, allowed_uids)

        # GT missing/unusable:
        return bool(args.include_gt_missing)

    orig_n = len(reports_df)
    reports_df = reports_df[reports_df.apply(should_keep_row, axis=1)].copy()
    filtered_n = len(reports_df)
    ...
    print(f"Loaded filter CSV: {args.filter_csv}")
    print(f"Filter UID column used: '{filter_uid_col_used}'")
    print(f"Allowed UIDs loaded: {len(allowed_uids)}")
    print(f"Reports before filtering: {orig_n}")
    print(f"Reports after filtering:  {filtered_n}")
    if args.include_gt_missing:
        print("NOTE: include_gt_missing=ON (reports missing in GT are included even though UID-filter cannot be applied).")
    else:
        print("NOTE: include_gt_missing=OFF (reports missing in GT are skipped).")

    print(f"\nProcessing {len(reports_df)} reports")
        ...
        for idx, row in reports_df.iterrows():
            ...
            if not report_text:
                minimal = {
                    ...
                    "_meta": {
                        ...
                        "source_filter_csv": str(args.filter_csv),
                        "filter_uid_col_used": filter_uid_col_used,
                        "allowed_uids_count": len(allowed_uids),
            ...
            try:
                ...
                if gt_entry and gt_entry.get("uids"):
                    ...
                    # Safety: this should already be true due to pre-filtering,
                    # but keep a guard in case include_gt_missing changed behavior.
                    if not report_has_any_allowed_uid(gt_entry, allowed_uids):
                        # Skip silently (or write a small skip note JSON)
                        skipped = {
                            "row_index": int(idx),
                            ANON_ACC_COL: anon_acc,
                            "skipped": True,
                            "reason": "Filtered out (no GT sequence_instance_uid matched allowed UID set).",
                            "_meta": {
                                "source_reports_csv": str(args.reports_csv),
                                "source_gt_csv": str(args.gt_csv),
                                "source_filter_csv": str(args.filter_csv),
                    ...
                    parsed["_meta"] = {
                        ...
                        "filtering": {
                            "enabled": True,
                            "matched_any_uid": True,
                        },
                    }

                else:
                    # GT missing for this Anon Acc #
                    if not args.include_gt_missing:
                        # (Should not happen due to pre-filtering) but keep safe behavior
                        skipped = {
                            "row_index": int(idx),
                            ANON_ACC_COL: anon_acc,
                            "skipped": True,
                            "reason": "GT not found; skipped because include_gt_missing is OFF.",
                            "_meta": {
                                "source_reports_csv": str(args.reports_csv),
                                "source_gt_csv": str(args.gt_csv),
                                "source_filter_csv": str(args.filter_csv),
                    ...
                    # If include_gt_missing ON, fallback extraction/QA can run
                    ...
                    if not isinstance(notes, list):
                        notes = []
                    notes.insert(0, "GT not found for this Anon Acc #; model estimate used. UID-based filtering could not be applied.")
                    parsed["notes"] = notes
                    ...
                    parsed["_meta"] = {
                        ...
                        "filtering": {
                            "enabled": True,
                            "matched_any_uid": None,
                            "note": "GT missing; cannot check UID intersection.",
                        },
                    }

                    # Ensure 'qa' exists for each sequence in fallback too
    ...
    print("Done ✔ One JSON file per (filtered) report has been written.")
